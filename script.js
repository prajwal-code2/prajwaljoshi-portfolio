...
const vertices = new Float32Array([
  ...
  0,
  0,
  vLength * Math.cos(vAngle),
  vLength * Math.sin(vAngle),
  0,
  vLength * Math.cos(-vAngle),
  vLength * Math.sin(-vAngle),
  0,
...
const vShapeMaterial = new THREE.MeshBasicMaterial({
  color: 0xff0000,
  transparent: true,
  opacity: 0.3,
...
const countSpriteMaterial = new THREE.SpriteMaterial({
  map: countTexture,
  transparent: true,
});
...
// Typewriter Effect - Now starting from left edge and slightly larger
...
typewriterCanvas.style.margin = "0 auto";
const typewriterCtx = typewriterCanvas.getContext("2d");
typewriterCtx.font = "56px Exo 2";
  ...
  ships.forEach((ship, index) => {
    ...
    if (!ship.userData.detected) {
      const relativePos = new THREE.Vector3().subVectors(
        ship.position,
        scannerField.position
      );
      const vDirection = new THREE.Vector3(1, 0, 0).applyQuaternion(
        scannerField.quaternion
      );
      ...
      if (angle < vAngle && distance <= vLength) {
        ...
        const detectSpriteMaterial = new THREE.SpriteMaterial({
          map: detectTexture,
          transparent: true,
        });
        ...
        if (robotModel) {
          detectSprite.position.set(
            robotModel.position.x,
            robotModel.position.y + 3,
    ...
    if (ship.userData.detected && ship.userData.detectionTime) {
      ...
      if (elapsed >= 3000) {
        ...
        if (ship.userData.detectSprite) {
          if (robotModel) {
            ship.userData.detectSprite.position.set(
              robotModel.position.x,
              robotModel.position.y + 3,
              robotModel.position.z
            );
